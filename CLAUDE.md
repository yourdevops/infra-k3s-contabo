...
External traffic flows through: **Client -> DNS (per-host A records managed by external-dns) -> Traefik (LoadBalancer, ports 80/443 via k3s ServiceLB) -> services**. TLS is terminated at Traefik using a wildcard Let's Encrypt certificate (`*.yourdevops.me`) obtained by cert-manager via DNS-01 challenge against Cloudflare. Internal traffic (Traefik -> services) is plain HTTP.

Routing uses the Kubernetes Gateway API: Traefik's Helm chart creates the `GatewayClass` + `Gateway` with HTTP/HTTPS listeners (including Gateway API CRDs). `HTTPRoute` resources in service namespaces attach to the Gateway. external-dns watches HTTPRoute hostnames and auto-creates/deletes A records in Cloudflare. The Cloudflare Zero Trust Tunnel and cloudflared pods remain deployed but idle — reserved for future WARP-only private services.
...
3. k3s install (delegates to `k3s.orchestration.site` collection — k3s built-in Traefik disabled (managed via ArgoCD instead), ServiceLB enabled)
